...
A service that exposes a simple `generate(prompt: string, model: string): Effect<string, LlmApiError>` method that:
...
- [ ] Test layer allows mocking LLM responses
- [ ] Integration test demonstrates real API call (can be skipped in CI)
...
```yaml
Task 1: Create LlmApi service definition
...
Task 2: Create LlmApi layer implementation
...
const makeService = Effect.gen(function* () {
  ...
  const generate = (prompt: string, model: string) =>
    Effect.gen(function* () {
      ...
      const request = provider === 'openai' 
        ? buildOpenAIRequest(prompt, model, providerConfig)
        : buildAnthropicRequest(prompt, model, providerConfig);
...
export const LlmApiTest = makeTestLayerFor(LlmApiService)({
  generate: (prompt, model) => {
    ...
    return Effect.succeed(`Generated response for: ${prompt} using ${model}`);
  },
});
```
...
it.effect('should generate text successfully', () =>
  Effect.gen(function* () {
    ...
    const result = yield* llmApi.generate('Hello world', 'gpt-4');
...
it.effect('should handle API errors', () =>
  Effect.gen(function* () {
    ...
    const exit = yield* Effect.exit(
      llmApi.generate('trigger error', 'gpt-4')
    );
...
it.effect('should select correct provider based on model', () =>
  Effect.gen(function* () {
    const llmApi = yield* LlmApiService;
    // Test OpenAI model
    const gptResult = yield* llmApi.generate('test', 'gpt-4');
    expect(gptResult).toContain('gpt-4');
    
    // Test Anthropic model
    const claudeResult = yield* llmApi.generate('test', 'claude-3-opus');
...
export LLM_OPENAI_MODEL=gpt-4

# Run integration test (skipped in CI)
