...
describe('TestCaseBuilder', () => {
  describe('buildConversationFromTestCase', () => {
    ...
    it.effect(
      'should build multi-turn conversations with parameter substitution',
      () =>
        Effect.gen(function* () {
          ...
          const conversationTest: TestCase = {
            id: Schema.decodeSync(
              Schema.String.pipe(Schema.brand('TestCaseId')),
            )('323e4567-e89b-12d3-a456-426614174002'),
            name: 'Support conversation',
            description:
              'Multi-turn support conversation with parameter substitution',
            createdAt: Schema.decodeSync(Schema.DateTimeUtc)(
              '2024-01-01T00:00:00Z',
            ),
            llmModel: Schema.decodeSync(LLMModel)('gpt-4'),
          ...
          const conversation =
            yield* ContentService.buildConversationFromTestCase(
              conversationTest,
            );
          ...
          expect(Chunk.toReadonlyArray(conversation)).toEqual([
            ...
            {
              role: 'user',
              content: 'I need help with TypeScript and generic types',
            },
          ]);
    ...
    it.effect('should handle parameter substitution in conversations', () =>
      Effect.gen(function* () {
        ...
        const testCase: TestCase = {
          ...
          llmModel: Schema.decodeSync(LLMModel)('gpt-4'),
          messageSlots: [{ role: 'user', tags: ['help-message'], sequence: 0 }],
          // Note: TestCase parameters don't affect substitution -
