...
describe("Snippet", () => {
  ...
  it.effect("should reject snippet with invalid id", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.either(Schema.decode(Snippet)(invalidSnippet))
...
describe("SnippetVersion", () => {
  ...
  it.effect("should decode valid snippet version", () =>
    Effect.gen(function* () {
      ...
      expect(JSON.stringify(result.createdAt)).toBe('"2024-01-01T00:00:00.000Z"')
    })
  )
  ...
  it.effect("should accept template content with multiple variables", () =>
    Effect.gen(function* () {
      const validVersion = {
        id: "456e7890-e89b-42d3-a456-426614174001",
        content: "As a {{role}}, you {{obligation_level}} {{action}} when {{condition}}",
        createdAt: validDate,
  ...
  it.effect("should accept plain text content without variables", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Schema.decode(SnippetVersion)(validVersion)
      expect(result.content).toBe("This is plain text without any template variables")
      expect(JSON.stringify(result.createdAt)).toBe('"2024-01-01T00:00:00.000Z"')
    })
  )
  ...
  it.effect("should reject version with invalid date", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.either(Schema.decode(SnippetVersion)(invalidVersion))
      expect(result._tag).toBe("Left")
    })
  )
